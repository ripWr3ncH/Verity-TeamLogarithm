...
#
#  Note officer-nasrin at seniority 3 and officer-kamal ALSO at 2 — the equal
#  seniority case is red-team attack #2, and it needs a real identity to fail
#  with.
# ---------------------------------------------------------------------------
USERS=(
...
registerAndEnrol() {
  ...
  mkdir -p "$userHome"
  FABRIC_CA_CLIENT_HOME="$userHome" fabric-ca-client enroll \
    -u "http://${user}:${user}pw@localhost:${caPort}" \
    --caname "$caName" \
    --enrollment.attrs "role,seniority,institution,displayName" \
    -M "${userHome}/msp" >/dev/null 2>&1 \
    || die "could not enrol ${user}"
